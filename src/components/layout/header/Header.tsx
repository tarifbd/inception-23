...
export function Header({ navigation, menuItems }: { navigation?: CollectionRecord[]; menuItems?: Partial<Record<MenuKind, CmsMenuItem[]>> }) {
    ...
    <>
      <header
        ...
        className={`fixed inset-x-0 top-0 z-[80] transition-all duration-300 ${
          scrolled
            ? 'bg-[#f7f9f8]/94 py-3 shadow-[0_1px_0_rgba(148,163,184,0.16)] backdrop-blur-2xl dark:bg-[#080b0e]/94 dark:shadow-[0_1px_0_rgba(255,255,255,0.08)]'
            : 'py-5'
        }`}
      >
          ...
          >
            <Link href="/" className="group ml-1 flex shrink-0 items-center gap-3 focus:outline-none focus:ring-4 focus:ring-brand-700/15 sm:ml-2 2xl:ml-3">
              ...
              <span className="hidden min-w-0 leading-none lg:block">
                <span className="block whitespace-nowrap text-[0.82rem] font-black uppercase tracking-[0.18em] text-brand-950 dark:text-white min-[1900px]:text-[1.05rem] min-[1900px]:tracking-[0.24em]">
                  Inception 23
                </span>
            ...
            <div className="flex shrink-0 items-center gap-2">
              <HeaderToggles className="hidden 2xl:flex" />
