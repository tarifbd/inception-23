...
export const CaseStudies = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const catRes = await fetch('/api/config');
      if (catRes.ok) {
        const data = await catRes.json();
        setCategories(data);
      }

      const studiesRes = await fetch('/api/casestudies');
      if (studiesRes.ok) {
        const data = await studiesRes.json();
        setCaseStudies(data);
      }
    } catch (err) {
      console.error('CaseStudies config fetch error:', err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('config-updated', fetchData);
    return () => {
      window.clearTimeout(initialFetch);
      window.removeEventListener('config-updated', fetchData);
    };
  }, [fetchData]);
                ...
                <TiltCard className="h-full">
                  <div className="relative group bg-white dark:bg-night-900 border border-gray-200/50 dark:border-white/5 rounded-[2rem] overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 flex flex-col h-full">
                    ...
                    <div className="relative w-full aspect-[16/10] overflow-hidden bg-black">
                      <Image src={study.img} alt={study.titleEn} fill className="object-cover scale-100 group-hover:scale-105 transition-transform duration-[1.5s]" unoptimized />
                    ...
                    <div className="p-8 md:p-10 flex flex-col justify-between flex-grow">
                      <div>
                        <span className="text-[10px] font-black tracking-widest text-brand-650 dark:text-brand-400 uppercase block mb-2">
            ...
            >
              ...
              <div className="overflow-y-auto no-scrollbar flex-grow">
                ...
                <div className="relative w-full aspect-[16/8] bg-black">
                  <Image src={selectedCase.img} alt={selectedCase.titleEn} fill className="object-cover" unoptimized />
