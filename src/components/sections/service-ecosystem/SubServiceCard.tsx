...
export function SubServiceCard({ service, serviceKey, index }: SubServiceCardProps) {
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -4 }}
      className={`group relative min-h-[148px] overflow-hidden rounded-[1.35rem] border bg-white/84 p-5 shadow-sm backdrop-blur-xl transition hover:shadow-xl hover:shadow-slate-950/10 ${theme.border}`}
    >
      ...
      <div className="relative flex items-start justify-between gap-4">
        <h4 className="text-base font-black leading-snug text-brand-950">{service.title}</h4>
      ...
      <p className="relative mt-4 text-sm leading-7 text-slate-600">{service.description}</p>
    </motion.article>
  );
}
