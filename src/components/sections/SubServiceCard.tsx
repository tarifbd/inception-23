...
export function SubServiceCard({ title, summary, serviceKey, index = 0 }: SubServiceCardProps) {
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -4 }}
      className={`group min-h-[118px] rounded-[1.25rem] border bg-white/82 p-4 shadow-sm backdrop-blur-xl transition hover:shadow-xl hover:shadow-slate-950/10 ${theme.border}`}
    >
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-sm font-black leading-snug text-brand-950">{title}</h3>
